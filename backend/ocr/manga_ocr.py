...
Usage:
    ...
    ocr = MangaOCR(backend="easyocr", languages=["ja", "en"])
...
class MangaOCR:
    def __init__(
        ...
        if backend == "easyocr":
            ...
            raise ValueError(f"Unknown backend: {backend!r}. Use 'easyocr' or 'tesseract'.")
...
# Run with:  uvicorn manga_ocr:app --reload
# --------------------------------------------------------------------------
...
def build_app(
    ...
    async def ocr_page_with_overlay(
        ...
        out_path = os.path.join(UPLOAD_DIR, f"translated_{file.filename}")
...
# module-level app for `uvicorn manga_ocr:app` convenience; only built when
# FastAPI is actually installed, so importing this module elsewhere doesn't
# hard-require it.
try:
    app = build_app()
except ImportError:
...
def main() -> None:
    ...
    page_p.add_argument("--backend", default="easyocr", choices=["easyocr", "tesseract"])
    ...
    chap_p.add_argument("--backend", default="easyocr", choices=["easyocr", "tesseract"])
    ...
    serve_p.add_argument("--backend", default="easyocr", choices=["easyocr", "tesseract"])
